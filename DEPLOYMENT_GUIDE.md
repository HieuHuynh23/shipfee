...
|---------|---------|
| **Repository** | `github.com/HieuHuynh23/shipfee` |
| **Branch** | `main` (Production) — sync từ `master` sau mỗi merge |
| **Framework Preset** | Other |
| **Root Directory** | (trống — root của repo) |
| **Build Command** | `npm run build` (`node scripts/build.js`) |
| **Output Directory** | `public` |

> **Quan trọng:** Vercel Production Branch = **`main`**. Render API Branch = **`master`**. Sau khi merge PR vào `master`, luôn chạy:
> `git fetch origin && git push origin origin/master:main`
> để production frontend cập nhật. Preview trên PR branch không phải production.

#### Mục tiêu một nhánh production (khuyến nghị)

Hiện tại vẫn hai nhánh (`master` API + `main` frontend). Để bỏ footgun sync:

1. Đặt **Render** Branch = `main` (hoặc cả hai platform cùng `master`).
...
```bash
# Root package.json — cross-platform (Windows + Vercel Linux)
npm run build
# = node scripts/build.js → copy customer-app, shipper-app, admin-app → public/
```
